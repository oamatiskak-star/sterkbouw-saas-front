-- Reviews idempotent maken (geen pile-up bij rerun) + bestaande dubbele assembly-reviews opruimen.
-- 1) cleanup: hou per logische sleutel de oudste open assembly-review
delete from harvest.review_queue a
using harvest.review_queue b
where a.reden like 'assembly-%'
  and b.reden = a.reden
  and coalesce(a.details->>'ifc_guid','')   = coalesce(b.details->>'ifc_guid','')
  and coalesce(a.details->>'functie','')    = coalesce(b.details->>'functie','')
  and coalesce(a.details->>'combi_code','') = coalesce(b.details->>'combi_code','')
  and a.created_at > b.created_at;

-- 2) engine: review-inserts met NOT EXISTS-guard (idempotent)
...
begin
  ...
  if not found then
    ...
    select 'assembly-geen-template',
      jsonb_build_object('ifc_object_id',v_obj.id,'ifc_guid',v_obj.ifc_guid,'ifc_entity',v_obj.ifc_entity)
    where not exists (select 1 from harvest.review_queue q
      where q.reden='assembly-geen-template' and q.details->>'ifc_guid'=v_obj.ifc_guid and q.status='open');
  ...
  if v_tpl.route = 'rekenmodel' then
    insert into harvest.review_queue(reden, details)
    select 'assembly-route-rekenmodel-buiten-fase2',
    ...
    where not exists (select 1 from harvest.review_queue q
      where q.reden='assembly-route-rekenmodel-buiten-fase2' and q.details->>'ifc_guid'=v_obj.ifc_guid and q.status='open');
  ...
  for it in select * from assembly.template_item
    ...
    if v_raw is null or v_raw !~ '^-?[0-9.]+$' then
      insert into harvest.review_queue(reden, details)
      select 'assembly-quantity-ontbreekt',
      ...
      where not exists (select 1 from harvest.review_queue q
        where q.reden='assembly-quantity-ontbreekt' and q.details->>'ifc_guid'=v_obj.ifc_guid
          and q.details->>'functie'=it.functie and q.details->>'combi_code'=it.combi_code and q.status='open');
...
select 'reviews idempotent + opgeruimd' as status;
