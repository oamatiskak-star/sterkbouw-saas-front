    ...
    <section className="mx-auto max-w-5xl px-6 py-4 sm:px-8" id="werkzaamheden">
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        <div className="mt-5 divide-y divide-gray-100">
          {hoofdstukken.map((h) => (
            <div key={h.id}>
              <button onClick={() => setOpen(open === h.id ? null : h.id)} className="flex w-full flex-col gap-1.5 bg-white py-3 text-left [color-scheme:light]">
                <span className="flex items-center gap-3">
                  ...
                  <span className="min-w-0 flex-1 text-sm font-semibold leading-snug text-gray-900">{h.naam}</span>
                  <ChevronDown size={16} className={`shrink-0 text-gray-400 transition-transform ${open === h.id ? 'rotate-180' : ''}`} />
                </span>
                <span className="flex items-center justify-between pl-14 text-xs text-gray-400">
                  <span>{h.rows.length} werkregel{h.rows.length === 1 ? '' : 's'}</span>
                  <span className="text-sm font-semibold text-gray-900">{fmtEUR(h.bedrag)}</span>
                </span>
              </button>
              {open === h.id && (
