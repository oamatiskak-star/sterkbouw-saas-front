...
import { ChevronLeft, Loader2, FileText, Send, Check, FilePlus2 } from 'lucide-react';
import supabase from '@/lib/supabase';
import { calcData, maakOfferte, updateOfferte } from '@/services/calcModules';
...
export default function OffertePagina() {
  ...
  useEffect(() => {
    if (!id) return;
    (async () => {
      try { const d = await calcData(id); setTotalen(d.totalen); await herlaad(); } finally { setLoading(false); }
    })();
  }, [id]);
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-3">
          <div className="md:col-span-2 space-y-4">
            ...
            <div className="flex gap-2">
              ...
              {offerte.status === 'verzonden' && <button onClick={() => zetStatus('getekend')} disabled={busy} className="inline-flex items-center gap-1.5 rounded-lg bg-sterkcalc-accent px-4 py-2 text-sm font-medium text-white"><Check size={15} /> Markeer als getekend</button>}
            </div>
          </div>
