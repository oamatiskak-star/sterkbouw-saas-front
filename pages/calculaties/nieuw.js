...
const EXECUTOR_URL="https://sterkbouw-saas-executor-production.up.railway.app"

const styles={
wrap:{maxWidth:1200,margin:"0 auto",padding:24},
grid4:{display:"grid",gridTemplateColumns:"1fr 1fr",gridTemplateRows:"420px 420px",gap:24},
card:{border:"1px solid #e5e7eb",borderRadius:8,padding:16,background:"#fff",display:"flex",flexDirection:"column",height:"100%",boxSizing:"border-box"},
cardTitle:{fontWeight:600,marginBottom:12},
fieldGrid:{display:"grid",gap:12},
label:{fontSize:13},
input:{width:"100%",padding:10,borderRadius:6,border:"1px solid #d1d5db",fontSize:14},
button:{width:"100%",padding:14,borderRadius:6,border:"none",background:"#2563eb",color:"#fff",fontSize:15,cursor:"pointer"},
preview:{flex:1,borderRadius:6,border:"1px solid #d1d5db",overflow:"hidden",background:"#000"},
...
const[processStatus,setProcessStatus]=useState({fase:"Wachten",actie:null})
...
const[opslagen,setOpslagen]=useState({ak_pct:0.08,abk_pct:0.04,w_pct:0.06,r_pct:0.05})
...
const avg=uurlonen.reduce((s,u)=>s+u.uurloon,0)/uurlonen.length
const arbeid=100*avg
const materiaal=250
const sub=arbeid+materiaal
return Math.round(sub+sub*opslagen.ak_pct+sub*opslagen.abk_pct+sub*opslagen.w_pct+sub*opslagen.r_pct)
}

async function saveOpslagen(pid){
await supabase.from("calculatie_correcties").upsert({project_id:pid,...opslagen})
}
...
setError(null)
try{
const res=await fetch("/api/projecten",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(form)})
...
finally{setCreating(false);createProjectGuardRef.current=false}
}
...
try{
const{data:project}=await supabase.from("projects").select("pdf_url").eq("id",projectId).maybeSingle()
if(project?.pdf_url&&!pdfUrl)setPdfUrl(project.pdf_url)
const{data:task}=await supabase.from("executor_tasks").select("action").eq("project_id",projectId).order("created_at",{ascending:false}).limit(1).maybeSingle()
if(task){
let fase="Wachten"
...
}finally{intervalTickGuardRef.current=false}
},3000)
return()=>{clearInterval(interval);intervalRunningRef.current=false}
...
<input style={styles.input}type="number"step="0.01"value={opslagen[k]}
onChange={e=>setOpslagen(p=>({...p,[k]:Number(e.target.value)}))}/>
</div>
))}
</div>
</div>
...
<input style={styles.input}type="number"value={u.uurloon}
onChange={e=>{const c=[...uurlonen];c[i].uurloon=Number(e.target.value);setUurlonen(c)}}/>
</div>
))}
</div>
</div>
...
<select style={styles.input}value={form.project_type}onChange={e=>updateForm("project_type",e.target.value)}>
<option value="nieuwbouw">nieuwbouw</option>
<option value="transformatie">transformatie</option>
<option value="renovatie">renovatie</option>
<option value="verduurzaming">verduurzaming</option>
</select>
{Object.keys(form).filter(k=>k!=="project_type").map(k=>(
<div key={k}>
<label style={styles.label}>{k}</label>
<input style={styles.input}value={form[k]}onChange={e=>updateForm(k,e.target.value)}/>
</div>
))}
</div>
</div>
...
{pdfUrl?<iframe title="preview"src={pdfUrl}style={styles.iframe}/>:
<div style={{color:"#9ca3af",padding:16}}>Nog geen calculatie</div>}
</div>
</div>
</div>

<div style={{marginTop:24}}><strong>Indicatie totaal:</strong> € {indicatie}</div>
