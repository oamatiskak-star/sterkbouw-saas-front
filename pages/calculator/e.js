...
export default function ECalculator() {
  ...
  useEffect(() => {
    axios.get("/api/calculator-e").then((res) => setItems(res.data))
  }, [])
  ...
  const handleSubmit = async (e) => {
    ...
    setItems(res.data)
    setFormData({ project_id: "", omschrijving: "", eenheden: 0, prijs_per_eenheid: 0 })
  }
  ...
  return (
    <div className="p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 mb-6">
        ...
        <input type="number" name="eenheden" placeholder="Aantal eenheden" value={formData.eenheden} onChange={handleChange} className="border p-2 w-full" required />
        <input type="number" name="prijs_per_eenheid" placeholder="Prijs per eenheid" value={formData.prijs_per_eenheid} onChange={handleChange} className="border p-2 w-full" required />
        <button type="submit" className="bg-yellow-600 text-white px-4 py-2 rounded">Toevoegen</button>
      </form>
      ...
      <div className="space-y-2">
        {items.map((item) => (
          <div key={item.id} className="border p-3 rounded shadow">
            ...
            <div>{item.eenheden} x €{item.prijs_per_eenheid} = €{item.eenheden * item.prijs_per_eenheid}</div>
          </div>
        ))}
      </div>
    </div>
