-- IFC candidate ranking V1.0 — scoort combi-kandidaten per staged IFC-object.
-- 7 criteria (gewogen): entity/omschrijving (0.45) · STABU-cat (0.25) · NL-SfB (0.10 basis)
...
scored as (
  select *,
    round((0.45*(case when combi_naam ~* kw then 1 else 0 end)
         ...
         + 0.05*coalesce(confidence,0))::numeric, 3) as score,
    concat_ws(' + ',
      case when combi_naam ~* kw then 'entity/omschrijving' end,
      case when category_code = any(cats) then 'STABU-cat' end, 'NL-SfB',
      case when (eenheid in ('m²','m2') and heeft_area) or (eenheid ~* '^(st|stuk)' and ifc_entity in ('IfcWindow','IfcDoor')) then 'eenheid' end,
      case when propertyset <> '{}'::jsonb and propkw <> '()' and combi_naam ~* propkw then 'property' end) as reason
  from cand
),
ranked as (select *, row_number() over (partition by obj_id order by score desc, code) rn from scored)
