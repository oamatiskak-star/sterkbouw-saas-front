...
export async function genereerOffertePdf({ offerte, totalen, calculatie, chapters = [], rows = [], settings = {}, download = true }) {
  ...
  if (logo) {
    const logoW = 40, logoH = logo.w && logo.h ? logoW * (logo.h / logo.w) : 21;
    try { doc.addImage(logo.dataUrl, logo.fmt, M, 154, logoW, logoH); } catch { /* skip */ }
  } else {
  ...
  // ============ DEEL 3A — VOLLEDIGE BESTEKLIJST (calculatie-module) ============
  // Op verzoek: elke offerte krijgt voortaan standaard de complete regel-voor-regel
  // calculatie als bijlage, niet alleen de hoofdstuk-samenvatting hierboven.
  if (offerte?.modules?.calculatie && rows?.length) {
    sectie('Bijlage — volledige besteklijst');
    doc.setFont('helvetica', 'normal'); doc.setFontSize(8.5); doc.setTextColor(120);
    doc.text('Alle regels waarop de prijsopgave is gebaseerd, per hoofdstuk (kostprijs excl. opslagen/btw).', M, y);
    y += 7;
    const chapterMap = new Map((chapters || []).map((c) => [c.id, c.naam || c.code || 'Hoofdstuk']));
    const perChapterRows = new Map();
    for (const r of rows) {
      const key = r.chapter_id || '__overig';
      if (!perChapterRows.has(key)) perChapterRows.set(key, []);
      perChapterRows.get(key).push(r);
    }
    const chapterOrder = [...(chapters || []).map((c) => c.id), ...(perChapterRows.has('__overig') ? ['__overig'] : [])];
    for (const chId of chapterOrder) {
      const chRows = perChapterRows.get(chId);
      if (!chRows?.length) continue;
      need(11);
      doc.setFont('helvetica', 'bold'); doc.setFontSize(10); doc.setTextColor(accent[0], accent[1], accent[2]);
      doc.text(String(chapterMap.get(chId) || 'Overig').slice(0, 90), M, y); y += 5.5;
      doc.setFont('helvetica', 'normal'); doc.setFontSize(8); doc.setTextColor(120);
      doc.text('Omschrijving', M, y);
      doc.text('Hoeveelh.', M + 108, y);
      doc.text('Mat./eh', M + 133, y, { align: 'right' });
      doc.text('Arb./eh', M + 155, y, { align: 'right' });
      doc.text('Regeltotaal', W - M, y, { align: 'right' });
      y += 4.5; doc.setDrawColor(220); doc.line(M, y, W - M, y); y += 4;
      let chSub = 0;
      doc.setTextColor(40);
      for (const r of chRows) {
        ...
        chSub += cr.kostprijs;
        need(6.5);
        const oms = doc.splitTextToSize(String(r.omschrijving || ''), 105);
        doc.setFontSize(8); doc.setFont('helvetica', 'normal');
        doc.text(oms[0] || '', M, y);
        doc.text(`${cr.hoeveelheid} ${r.eenheid || ''}`.trim(), M + 108, y);
        doc.text(eur(cr.unit.materiaalprijs), M + 133, y, { align: 'right' });
        doc.text(eur(cr.unit.arbeidsprijs), M + 155, y, { align: 'right' });
        doc.text(eur(cr.kostprijs), W - M, y, { align: 'right' });
        y += 5;
        for (const extra of oms.slice(1, 2)) { need(5); doc.text(extra, M, y); y += 4.5; }
      }
      need(7); doc.setDrawColor(220); doc.line(M, y, W - M, y); y += 5;
      doc.setFont('helvetica', 'bold'); doc.setFontSize(8.5);
      doc.text(`Subtotaal ${String(chapterMap.get(chId) || 'Overig').slice(0, 60)}`, M, y);
      doc.text(eur(chSub), W - M, y, { align: 'right' }); y += 8;
    }
  }

  // ============ DEEL 3B — ONTWERP (plattegrond/3D-voorstel) ============
  const ontwerpItems = Array.isArray(cover.ontwerpAfbeeldingen) ? cover.ontwerpAfbeeldingen.filter((o) => o?.url) : [];
  if (ontwerpItems.length) {
    for (const item of ontwerpItems) {
      const img = await loadImage(item.url);
      if (!img) continue;
      doc.addPage(); y = M;
      sectie(item.titel || 'Ontwerpvoorstel');
      const maxW = W - 2 * M, maxH = H - y - 26;
      let iw = maxW, ih = maxW * (img.h && img.w ? img.h / img.w : 0.75);
      if (img.w && img.h) { const scale = Math.min(maxW / img.w, maxH / img.h); iw = img.w * scale; ih = img.h * scale; }
      else if (ih > maxH) { ih = maxH; iw = maxH / 0.75; }
      const ix = M + (maxW - iw) / 2;
      try { doc.addImage(img.dataUrl, img.fmt, ix, y, iw, ih); } catch { /* skip */ }
      y += ih + 6;
      if (item.beschrijving) {
        doc.setFont('helvetica', 'normal'); doc.setFontSize(9.5); doc.setTextColor(90);
        const lines = doc.splitTextToSize(item.beschrijving, W - 2 * M);
        need(lines.length * 4.5); doc.text(lines, M, y); y += lines.length * 4.5;
      }
    }
  }

  // ============ DEEL 3C — ADVIES ============
  const advies = Array.isArray(offerte?.content?.advies) ? offerte.content.advies.filter((a) => a?.titel) : [];
  if (advies.length) {
    sectie('Advies');
    for (const a of advies) {
      doc.setFont('helvetica', 'bold'); doc.setFontSize(10.5); doc.setTextColor(40);
      const titelW = a.bedrag ? W - 2 * M - 35 : W - 2 * M;
      const titelLines = doc.splitTextToSize(a.titel, titelW);
      need(titelLines.length * 5 + 2);
      doc.text(titelLines, M, y);
      if (a.bedrag) {
        doc.setFont('helvetica', 'bold'); doc.setFontSize(10.5); doc.setTextColor(accent[0], accent[1], accent[2]);
        doc.text(`+ ${eur(a.bedrag)}`, W - M, y, { align: 'right' });
      }
      y += titelLines.length * 5 + 3;
      if (a.tekst) {
        doc.setFont('helvetica', 'normal'); doc.setFontSize(9.5); doc.setTextColor(90);
        const lines = doc.splitTextToSize(a.tekst, W - 2 * M);
        need(lines.length * 4.5); doc.text(lines, M, y); y += lines.length * 4.5;
      }
      y += 4;
    }
  ...
  // ============ DEEL 6B — ZEKERHEDEN ============
  const zekerheden = Array.isArray(offerte?.content?.zekerheden) ? offerte.content.zekerheden : [];
  if (zekerheden.length) {
    sectie('Zekerheden');
    doc.setFont('helvetica', 'normal'); doc.setFontSize(9.5); doc.setTextColor(40);
    for (const z of zekerheden) { need(6.5); doc.text(`✓ ${z}`, M, y); y += 6.5; }
    y += 2;
  }
  ...
  need(38); doc.setDrawColor(220); doc.line(M, y, W - M, y); y += 7;
  ...
  if (offerte?.ondertekening?.naam || offerte?.getekend_at) {
    ...
    const handtekening = ot.handtekeningDataUrl ? await loadImage(ot.handtekeningDataUrl) : null;
    if (handtekening) { try { doc.addImage(handtekening.dataUrl, handtekening.fmt, M, y + 3, 45, 20); } catch { /* skip */ } }
    doc.setTextColor(20); doc.setFont('helvetica', 'bold');
    doc.text(`${ot.naam || offerte.klant_naam || ''} — digitaal getekend ${datNL(offerte.getekend_at)}${ot.tijd ? ' ' + ot.tijd : ''}`, M, y + (handtekening ? 28 : 7));
    doc.setFont('helvetica', 'normal'); doc.setTextColor(140); doc.setFontSize(8);
    const auditRegels = [ot.ip ? `IP ${ot.ip}` : null, ot.voorwaardenVersie ? `Voorwaarden v${ot.voorwaardenVersie}` : null, `Offerteversie ${offerte.versie || 1}`].filter(Boolean).join('  ·  ');
    if (auditRegels) doc.text(`Audittrail: ${auditRegels}`, M, y + (handtekening ? 33 : 12));
  } else {
  ...
  if (download) { doc.save(naam); return naam; }
  return { naam, blob: doc.output('blob') };
}
