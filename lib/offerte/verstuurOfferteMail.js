...
async function sendViaSmtp({ to, fromEmail, fromName, replyTo, subject, html, text }) {
  ...
  try {
    ...
    // 'sent: true' betekent alleen dat de Cloud86-mailserver het bericht heeft AANGENOMEN
    // voor bezorging (SMTP-response, accepted/rejected-adressen) — geen garantie dat het ook
    // daadwerkelijk in de inbox van de ontvanger is beland (spamfilters, externe MX's).
    return {
      sent: true,
      provider: 'smtp',
