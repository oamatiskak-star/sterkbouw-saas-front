...
function bouwRawMime({ from, to, replyTo, subject, html, text, attachments }) {
  const composer = new MailComposer({ from, to, replyTo: replyTo || undefined, subject, html, text, attachments: attachments || undefined });
  return new Promise((resolve, reject) => {
    composer.compile().build((err, message) => (err ? reject(err) : resolve(message)));
  });
}

async function sendViaSmtp({ smtp, to, fromEmail, fromName, replyTo, subject, html, text, attachments }) {
  ...
  let raw;
  try {
    raw = await bouwRawMime({ from, to, replyTo, subject, html, text, attachments });
  } catch {
    raw = null; // bouwen van de raw-versie mag de verzending zelf niet blokkeren
  }

  try {
    const info = await transporter.sendMail(raw ? { raw, envelope: { from: fromEmail, to } } : { from, to, replyTo: replyTo || undefined, subject, html, text, attachments });
...
export async function verstuurMail({ smtp, to, replyTo, subject, html, text, fromEmail, fromName, attachments }) {
  const msg = { smtp, to, fromEmail, fromName, replyTo, subject, html, text, attachments };
