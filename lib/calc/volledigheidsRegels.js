...
const REGELS = [
  {
    id: 'geen-abk', ernst: 'hoog', titel: 'Geen ABK / staartkosten',
    advies: 'Voeg de algemene bouwplaatskosten toe (rekenmodel ABK / Staartkosten) — anders mist de offerte 5-12% van de bouwsom.',
    relevant: () => true,
    voldaan: (c) => c.cats.has('00'),
  },
  {
  ...
  {
    id: 'geen-terrein', ernst: 'middel', titel: 'Geen terrein/riolering',
    advies: 'Vergeet bij nieuwbouw/uitbreiding het terrein (verharding/grondwerk) en de buitenriolering/HWA niet.',
    relevant: (c) => ['nieuwbouw', 'uitbreiding', 'appartementencomplex'].includes(c.projecttype),
    voldaan: (c) => c.cats.has('30') || c.cats.has('32'),
  },
  {
    id: 'geen-sloop', ernst: 'middel', titel: 'Geen sloop/strip',
    advies: 'Renovatie/transformatie begint vrijwel altijd met sloop/strippen (+ asbestcheck) — voeg het sloop-rekenmodel toe.',
    relevant: (c) => ['renovatie', 'transformatie', 'badkamer', 'woning'].includes(c.projecttype),
    voldaan: (c) => c.cats.has('02'),
  },
  {
    id: 'geen-staal', ernst: 'middel', titel: 'Geen staalconstructie',
    advies: 'Controleer of er staal (lateien/liggers/portalen) nodig is — bij nieuwbouw/uitbreiding/transformatie vaak vergeten en prijsbepalend.',
    relevant: (c) => ['nieuwbouw', 'uitbreiding', 'transformatie', 'appartementencomplex'].includes(c.projecttype),
    voldaan: (c) => c.cats.has('07'),
  },
  {
