...
function kostprijsPerFase(chapters, rows) {
  const chById = new Map((chapters || []).map((c) => [c.id, c]));
  const map = {};
  for (const row of rows || []) {
    const ch = chById.get(row.chapter_id);
    const fkey = faseVanCategorie(ch?.code);
    map[fkey] = (map[fkey] || 0) + computeRow(row).kostprijs;
  }
  return map;
}

function margePerHoofdstuk(chapters, rows, totalen) {
  ...
  for (const row of rows || []) {
    ...
    map.set(naam, (map.get(naam) || 0) + computeRow(row).kostprijs);
  }
...
function cashflow(chapters, rows, planning, offerte, kpi) {
  const fases = planning?.fases || [];
  const kpf = kostprijsPerFase(chapters, rows);
...
export function bouwRapportage({ totalen, chapters, rows, offerte, planning, bestellingen = [], versieSnapshots = [] }) {
  ...
  const margeHfd = margePerHoofdstuk(chapters, rows, totalen);
  ...
  return {
    ...
    cashflow: cashflow(chapters, rows, planning, offerte, kpi),
