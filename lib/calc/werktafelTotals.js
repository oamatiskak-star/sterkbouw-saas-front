...
import { prijspeilFactor } from './prijsindex';
...
// prijspeilfactor (increment 1b) = CBS inputprijsindex bouwkosten op de gekozen
// prijspeildatum ÷ het basis-prijspeil (zie ./prijsindex). Geen datum → 1 (actueel).
export function priceFactor(opslagen) {
  const rf = parseFloat(opslagen?.regiofactor);
  const regio = Number.isFinite(rf) && rf > 0 ? rf : 1;
  const peil = prijspeilFactor(opslagen?.prijspeildatum);
  return regio * (Number.isFinite(peil) && peil > 0 ? peil : 1);
}
