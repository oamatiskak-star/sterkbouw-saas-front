...
const FunderingModel = {
  ...
  advancedInputs: [
    ...
    { key: 'footprint_m2', label: 'Bebouwd oppervlak (werkvloer/grondwerk)', type: 'number', eenheid: 'm²', default: 0 },
  ],
  ...
  bereken(v) {
    ...
    // Bebouwd oppervlak voor werkvloer/grondwerk: indien meegegeven (uit casco/tekening)
    // het echte bebouwd oppervlak; anders de afgeleide footprint van de fundering zelf.
    const fpInput = num(v.footprint_m2, 0);
    // Gedeelde grond-/uitvoeringsposten (footprint-gedreven) voor strook/plaat/palen.
    const grondEnUitvoering = (footprint, betonM3) => {
      const fp = fpInput > 0 ? fpInput : footprint;
      if (ja(v.werkvloer)) regels.push({ combiCode: 'P5-F416', hoeveelheid: fp, omschrijving: 'Werkvloer (schraal beton)' });
      if (wilGrondverb) regels.push({ combiCode: 'P5-F307', hoeveelheid: round(fp * cunet, 2), omschrijving: 'Grondverbetering' });
      if (ja(v.verdichting)) regels.push({ combiCode: 'P5-F311', hoeveelheid: fp, omschrijving: 'Verdichten' });
    ...
    if (v.type === 'palen') {
      // Complete heipalen-fundering = heipalen + funderingsbalken (grondbalken op de palen)
      // + werkvloer/grondwerk over het bebouwd oppervlak. L = gebouwomtrek (uit casco/tekening).
      const n = Math.max(1, Math.round(num(v.aantal, 10)));
      const omtrek = L;
      const balkB = 0.3, balkH = 0.5; // grondbalk-doorsnede (m)
      const betonM3 = round(omtrek * balkB * balkH * verliesF, 2);
      const bekistingM2 = geenBekisting ? 0 : round(2 * omtrek * balkH, 2);
      const wapM2 = round(omtrek * balkB, 2);
      const footprint = fpInput > 0 ? fpInput : round(omtrek * balkB, 2);
      // 1) heipalen
      regels.push({ combiCode: 'CUR-0405', hoeveelheid: n, omschrijving: `Heipalen (${n} st)` });
      // 2) funderingsbalken op de palen
      if (betonM3 > 0) {
        regels.push({ combiCode: 'C3-0414', hoeveelheid: betonM3, omschrijving: `Funderingsbalken beton (${v.betonsoort})` });
        if (bekistingM2 > 0) regels.push({ combiCode: 'C3-0409', hoeveelheid: bekistingM2, omschrijving: `Bekisting funderingsbalken (${v.bekisting})` });
        regels.push({ combiCode: 'CUR-0410', hoeveelheid: wapM2, omschrijving: `Wapening funderingsbalken (${v.wapening})` });
        regels.push({ combiCode: 'P5-F417', hoeveelheid: betonM3, omschrijving: 'Betonpomp + transport' });
        if (stappen > 0) regels.push({ combiCode: 'P5-F418', hoeveelheid: round(betonM3 * stappen, 2), omschrijving: `Betonsoort-meerprijs (${v.betonsoort})` });
      }
      // 3) werkvloer + grondwerk over het bebouwd oppervlak (betonM3=0 → geen dubbele betonpomp)
      grondEnUitvoering(footprint, 0);
      hoeveelheden = { palen: n, beton_m3: betonM3, footprint_m2: footprint };
      samenvatting = [{ label: 'Aantal palen', waarde: n, eenheid: 'st' }, { label: 'Funderingsbalken beton', waarde: betonM3, eenheid: 'm³' }];
