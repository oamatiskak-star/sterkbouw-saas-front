...
import riolering from './riolering';
import plafond from './plafond';
import stucwerk from './stucwerk';
import isolatie from './isolatie';
import dakopening from './dakopening';
import verdiepingsvloer from './verdiepingsvloer';
import stelposten from './stelposten';
...
export const REKENMODELLEN = { abk, sloop, fundering, staal, gevel, metselwerk, kozijn, dak, dakopening, vloer, verdiepingsvloer, binnenwand, trap, balkon, galerij, lift, plafond, stucwerk, isolatie, badkamer, keuken, toilet, schilderwerk, elektra, sanitair, verwarming, ventilatie, energie, brandcompartiment, terrein, riolering, stelposten };
...
// Aannames-snapshot van een rekenmodel: de gebruikte inputwaarden met leesbare
// labels (select-opties als label, niet als code). Increment 4 — wordt op de
// gegenereerde werktafelregels meegeschreven (meta.aannames) voor reproduceerbaarheid:
// later is terug te zien WELKE keuzes/maatvoering tot deze regels hebben geleid.
export function aannamesVanModel(model, values = {}) {
  const v = metDefaults(model, values);
  const defs = [...(model.inputs || []), ...(model.advancedInputs || [])];
  const out = [];
  for (const d of defs) {
    const raw = v[d.key];
    if (raw == null || raw === '') continue;
    let waarde = raw;
    if (d.type !== 'number' && Array.isArray(d.opties)) {
      const opt = d.opties.find((o) => String(Array.isArray(o) ? o[0] : o) === String(raw));
      if (opt) waarde = Array.isArray(opt) ? opt[1] : opt;
    }
    out.push({ key: d.key, label: d.label, waarde: String(waarde), eenheid: d.eenheid || null, groep: d.groep || null });
  }
  return out;
}
